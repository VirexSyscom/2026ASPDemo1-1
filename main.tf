...
locals {
  ...
  name = {
    ...
    # Developer SKU 不需公用 IP，bastion_pip 已移除
...
# Developer SKU 不需要 AzureBastionSubnet，故改為條件式建立
...
############################################
# Bastion - Spoke-VNET-bastion（Developer SKU：無公用 IP）
#
# 注意事項：
#   1. Developer SKU 不需公用 IP，也不需 AzureBastionSubnet
#   2. 不支援 VNet peering，僅能連線至 Spoke-VNET 內的 VM
#      （連不到 UAT-Spoke-VNET 或 Hub 的 VM）
#   3. 一次僅支援單一連線，且僅能透過 Azure 入口網站瀏覽器連線
#      （無 native client / IP-Connect / 自訂連接埠 / 檔案傳輸）
#   4. 屬共用資源，微軟定位為 Dev/Test，不建議用於正式環境
#   5. 需確認部署區域支援 Developer SKU
############################################
...
resource "azurerm_bastion_host" "spoke" {
  ...
  # Developer SKU 僅支援 copy/paste，其餘功能需 Standard 以上
